...
def ground_qa_pair(data):
    ...
    return {"sent": s, "ans": a, "qc": question_concepts, "ac": None}
...
def ground(statement_path, cpnet_vocab_path, pattern_path, output_path, num_processes=1, debug=False):
    ...
    for index,item in enumerate(datas):
        ...
        sents.append(item["question"])

        # answers.append(item["choice"])
...
if __name__ == "__main__":
    if PATTERN_PATH is None:
        PATTERN_PATH = '/data/cpnet/matcher_patterns.json'
    # sents = 'Doctor would you find magazines along side many other printed works.'
    # answers = 'doctor'
    # result = ground_qa_pair(sents, answers)
    # print(result)
    # create_matcher_patterns("../data/cpnet/concept.txt", "./matcher_res.txt", True)
    ground("../truthfulqa.json", "/data/cpnet/concept.txt", "/data/cpnet/matcher_patterns.json", "./ground_res.jsonl", 10, True)
