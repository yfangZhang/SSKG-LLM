        ...
        for t in range(n_node):
            ...
            if cpnet.has_edge(s_c, t_c):
                for e_attr in cpnet[s_c][t_c].values():
                    if e_attr['rel'] >= 0 and e_attr['rel'] < n_rel:
                        relation_list.append({'head_entity':id2concept[s_c],'rel': id2relation[e_attr['rel']],'tail_entity':id2concept[t_c]})
                        adj[e_attr['rel']][s][t] = 1
    # cids += 1  # note!!! index 0 is reserved for padding
    adj = coo_matrix(adj.reshape(-1, n_node))
    return adj, cids,relation_list
...
def concepts_to_adj_matrices_2hop_all_pair(qc_ids, ac_ids):
    # qc_ids, ac_ids = data
    qa_nodes = set(qc_ids) | set(ac_ids)
    extra_nodes = set()
    for qid in qa_nodes:
    ...
    qmask = arange < len(qc_ids)
    amask = (arange >= len(qc_ids)) & (arange < (len(qc_ids) + len(ac_ids)))
    adj, concepts,relation_list = concepts2adj(schema_graph)
    return adj, concepts, qmask, amask,relation_list
...
TOKENIZER = RobertaTokenizer.from_pretrained('/models/roaberta-large')
LM_MODEL = RobertaForMaskedLMwithLoss.from_pretrained('/models/roaberta-large')
...
def serialize(obj):
    """Recursively serialize numpy int32 types to Python int."""
...
def generate_adj_data_from_grounded_concepts(grounded_path, cpnet_graph_path, cpnet_vocab_path, output_path, num_processes):
    ...
    with open(grounded_path, 'r', encoding='utf-8') as fin:
        ...
        for line in fin:
            ...
            _, _, _, _,relation_list =  concepts_to_adj_matrices_2hop_all_pair(q_ids, a_ids)
    ...
    #     pickle.dump(res, fout)
            ...
            with jsonlines.open(output_path, 'a') as w:
                relation_list = serialize(relation_list)
                if len(relation_list)>0:
                    i+=1
                    w.write({'index':i,"question":dic["question"],"answer":dic["answer"],'subkgs':relation_list})
                # w.write({'index':i})
            # with open(output_path, 'a') as fout:
            #     pickle.dump(res[0][0], fout)
