...
for index,item in enumerate(datas):
    ...
    message = {"conversation_id": index+1,
               "category": "kg_qa",
               "conversation": [{"human":"| Knowledge_Graph is:"+sub_kg_sequence+"| Question is:"+item['question']+"| Choice is:"+item["choice"] ,"assistant": item['answer']}],
               "dataset": "truthfulqa"
               }
    # with jsonlines.open("wiki_zsl/wiki_multi_test_sample{}.jsonl".format(n), 'a') as w:
    #     w.write(message)
    with jsonlines.open("../truthfulqa_withkgs0925.json", 'a') as w:
        w.write(message)
