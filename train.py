...
from component.mykgllm import MyKGLLM
from component.mykgllm_nocross import MyKGLLM_nocross
from component.kopa import KoPAWithAdapter
from component.graphencoder import load_glm_t5
...
def load_model(args, training_args,tokenizer):
    ...
    llm_config = AutoConfig.from_pretrained(args.model_name_or_path, trust_remote_code=True)
    kg_config = AutoConfig.from_pretrained(args.graphencoder_card, trust_remote_code=True)
    model = AutoModelForCausalLM.from_pretrained(args.model_name_or_path, **model_kwargs)
    graph_model, graph_tokenizer = load_glm_t5(args.graphencoder_card)
    ...
    kg_model = MyKGLLM(model=model,tokenizer=tokenizer,graphencoder_card=args.graphencoder_card,kg_dim=kg_config.d_model,llm_dim=llm_config.hidden_size)
    # kg_model = MyKGLLM_nocross(model=model,tokenizer=tokenizer,graphencoder_card=args.graphencoder_card,kg_dim=kg_config.d_model,llm_dim=llm_config.hidden_size)
    # kg_model = KoPAWithAdapter(model, num_prefix=1, kge_model='/root/zhangyf/KoPA-main/data1/data/CoDeX-S-rotate.pth')
...
def load_sft_dataset(args, tokenizer,graph_tokenizer,graph_model):
    ...
    if 'chatglm2' in args.model_name_or_path.lower():
        ...
        train_dataset = UnifiedSFTDataset(args.train_file, tokenizer, graph_tokenizer=graph_tokenizer,graph_model=graph_model,max_seq_length=args.max_seq_length, template=template)
...
def init_components(args, training_args):
    ...
    # 加载model
    components = load_model(args, training_args, tokenizer)
    model = components['model']
    kg_model = components['kg_model']
    graph_tokenizer = components['graph_tokenizer']
    graph_model = components['graph_model']
    ...
    if args.task_type == 'pretrain':
        ...
        train_dataset = load_sft_dataset(args, tokenizer,graph_tokenizer=graph_tokenizer,graph_model=graph_model)
        data_collator = SFTDataCollator(tokenizer,graph_tokenizer=graph_tokenizer,graph_model=graph_model,max_seq_length=args.max_seq_length)
    else:
...
def main():
    ...
    # 加载各种组件
    trainer,model,kg_model = init_components(args, training_args)
    ...
    old_state_dict = kg_model.state_dict
    kg_model.state_dict = (
    ...
    #
    # torch.save(model.state_dict(), 'model_state_dict.pth', _use_new_zipfile_serialization=True)
    model.save_pretrained(final_save_path,safe_serialization=True)
    torch.save(kg_model.kg_adapter_cross_attention, os.path.join(final_save_path, "kg_adapter.pth"))
